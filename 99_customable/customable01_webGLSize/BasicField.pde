class BasicField extends Field {
  ...
  private void bounceOfWalls(Particle particle) {
    float xmin = -1 + particle.size() / 2;
    float xmax = 1 - particle.size() / 2;
    float ymin = -1 + particle.size() / 2;
    float ymax = 1 - particle.size() / 2;
  ...
  private void throughOfWalls(Particle particle) {
    ...
    float ymin = -1 - particle.size() / 2;
    float ymax = 1 + particle.size() / 2;
    float shiftWidth = 2 + particle.size();
    float shiftHeight = 2 + particle.size();
    PVector pos = particle.position();
    PVector vel = particle.velocity();
    if (pos.x < xmin) {
      pos.x += shiftWidth;
